...
void QualityStatsTest::TestSrr490124Equality(const QualityStats &test, const char *context){
	// echo "count seg ref dom tile pos qual"; cat <(samtools view -q 10 ecoli-SRR490124-4pairs.sam | awk '(int($2%32/16)){print "@" substr($18,6,length($18)-5), $2; print $10; print "+";print $11}' | seqtk seq -r | awk '(1==NR%4){md=substr($1,2,length($0)-1); flag=$2}(2==NR%4){seq=$0}(0==NR%4){printf("%i ", flag);num=0;mult=1;for(i=length(md);0<i;i-=1){b=substr(md,i,1);if(b ~ /^[0-9]/){num+=b*mult;mult*=10}else{base=N;if("A"==b){base="T"}; if("C"==b){base="G"}; if("G"==b){base="C"}; if("T"==b){base="A"}; printf("%i%s", num, base); num=0; mult=1}}; print num, seq, $0}') <(samtools view -q 10 ecoli-SRR490124-4pairs.sam | awk '(!int($2%32/16)){print $2, substr($18,6,length($18)-5), $10, $11}' ) | awk 'BEGIN{for(n=0;n<256;n++)ord[sprintf("%c",n)]=n}{pos=0; num=0; for(i=1;i<=length($2);i+=1){b=substr($2,i,1);if(b ~ /^[0-9]/){num=num*10+b}else{while(0 <= num){pos += 1; num -= 1; if(0>num){ref=b}else{ref=substr($3,pos,1)}; if(0>num && NR%3 != 1){dom=substr($3,pos,1)}else{dom="N"}; print int($1%256/128), ref, dom, 0, pos-1, ord[substr($4,pos,1)]-33}; num=0}}; for(pos+=1;pos<=length($3);pos+=1){print int($1%256/128), substr($3,pos,1), "N", 0, pos-1, ord[substr($4,pos,1)]-33}}' | sort -n | uniq -c
	...
	EXPECT_EQ(8, test.base_quality_stats_per_tile_per_error_reference_.at(0).at(0).at(1)[0][70][2]) << "SRR490124-4pairs base_quality_stats_per_tile_per_error_reference_[0] not correct for " << context << '\n';
	EXPECT_EQ(8, test.base_quality_stats_per_tile_per_error_reference_.at(0).at(2).at(1)[0][86][2]) << "SRR490124-4pairs base_quality_stats_per_tile_per_error_reference_[0] not correct for " << context << '\n';
	...
	EXPECT_EQ(8, test.base_quality_stats_per_tile_per_error_reference_.at(1).at(3).at(4)[0][10][39]) << "SRR490124-4pairs base_quality_stats_per_tile_per_error_reference_[1] not correct for " << context << '\n';

	// echo "count seg ref dom tile pos rate"; cat <(samtools view -q 10 ecoli-SRR490124-4pairs.sam | awk '(int($2%32/16)){print "@" substr($18,6,length($18)-5), $2; print $10; print "+";print $11}' | seqtk seq -r | awk '(1==NR%4){md=substr($1,2,length($0)-1); flag=$2}(2==NR%4){seq=$0}(0==NR%4){printf("%i ", flag);num=0;mult=1;for(i=length(md);0<i;i-=1){b=substr(md,i,1);if(b ~ /^[0-9]/){num+=b*mult;mult*=10}else{base=N;if("A"==b){base="T"}; if("C"==b){base="G"}; if("G"==b){base="C"}; if("T"==b){base="A"}; printf("%i%s", num, base); num=0; mult=1}}; print num, seq, $0}') <(samtools view -q 10 ecoli-SRR490124-4pairs.sam | awk '(!int($2%32/16)){print $2, substr($18,6,length($18)-5), $10, $11}' ) | awk 'BEGIN{for(n=0;n<256;n++)ord[sprintf("%c",n)]=n}{pos=0; num=0; for(i=1;i<=length($2);i+=1){b=substr($2,i,1);if(b ~ /^[0-9]/){num=num*10+b}else{while(0 <= num){pos += 1; num -= 1; if(0>num){ref=b}else{ref=substr($3,pos,1)}; if(0>num && NR%3 != 1){dom=substr($3,pos,1);rate=100}else{dom="N";rate=0}; print int($1%256/128), ref, dom, 0, pos-1, rate}; num=0}}; for(pos+=1;pos<=length($3);pos+=1){print int($1%256/128), substr($3,pos,1), "N", 0, pos-1, 0}}' | sort -n | uniq -c
	EXPECT_EQ(8, test.error_rate_for_position_per_tile_per_error_reference_.at(0).at(1).at(2)[0][87][100]) << "SRR490124-4pairs error_rate_for_position_per_tile_per_error_reference_[0] not correct for " << context << '\n';
	EXPECT_EQ(8, test.error_rate_for_position_per_tile_per_error_reference_.at(0).at(2).at(4)[0][97][0]) << "SRR490124-4pairs error_rate_for_position_per_tile_per_error_reference_[0] not correct for " << context << '\n';
	EXPECT_EQ(8, test.error_rate_for_position_per_tile_per_error_reference_.at(1).at(0).at(4)[0][1][0]) << "SRR490124-4pairs error_rate_for_position_per_tile_per_error_reference_[1] not correct for " << context << '\n';
	EXPECT_EQ(8, test.error_rate_for_position_per_tile_per_error_reference_.at(1).at(3).at(4)[0][99][0]) << "SRR490124-4pairs error_rate_for_position_per_tile_per_error_reference_[1] not correct for " << context << '\n';
	// echo "count seg ref tile pos rate"; cat <(samtools view -q 10 ecoli-SRR490124-4pairs.sam | awk '(int($2%32/16)){print "@" substr($18,6,length($18)-5), $2; print $10; print "+";print $11}' | seqtk seq -r | awk '(1==NR%4){md=substr($1,2,length($0)-1); flag=$2}(2==NR%4){seq=$0}(0==NR%4){printf("%i ", flag);num=0;mult=1;for(i=length(md);0<i;i-=1){b=substr(md,i,1);if(b ~ /^[0-9]/){num+=b*mult;mult*=10}else{base=N;if("A"==b){base="T"}; if("C"==b){base="G"}; if("G"==b){base="C"}; if("T"==b){base="A"}; printf("%i%s", num, base); num=0; mult=1}}; print num, seq, $0}') <(samtools view -q 10 ecoli-SRR490124-4pairs.sam | awk '(!int($2%32/16)){print $2, substr($18,6,length($18)-5), $10, $11}' ) | awk 'BEGIN{for(n=0;n<256;n++)ord[sprintf("%c",n)]=n}{pos=0; num=0; for(i=1;i<=length($2);i+=1){b=substr($2,i,1);if(b ~ /^[0-9]/){num=num*10+b}else{while(0 <= num){pos += 1; num -= 1; if(0>num){ref=b}else{ref=substr($3,pos,1)}; if(0>num && NR%3 != 1){rate=100}else{rate=0}; print int($1%256/128), ref, 0, pos-1, rate}; num=0}}; for(pos+=1;pos<=length($3);pos+=1){print int($1%256/128), substr($3,pos,1), 0, pos-1, 0}}' | sort -n | uniq -c
	EXPECT_EQ(8, test.error_rate_for_position_per_tile_reference_.at(0).at(1)[0][0][0]) << "SRR490124-4pairs error_rate_for_position_per_tile_reference_[0] wrong for " << context << '\n';
	...
	EXPECT_EQ(9, test.error_rate_for_position_per_tile_reference_.at(0).at(0)[0][99][0]) << "SRR490124-4pairs error_rate_for_position_per_tile_reference_[0] wrong for " << context << '\n';
	EXPECT_EQ(1, test.error_rate_for_position_per_tile_reference_.at(0).at(3)[0][93][0]) << "SRR490124-4pairs error_rate_for_position_per_tile_reference_[0] wrong for " << context << '\n';
	EXPECT_EQ(1, test.error_rate_for_position_per_tile_reference_.at(1).at(0)[0][99][0]) << "SRR490124-4pairs error_rate_for_position_per_tile_reference_[1] wrong for " << context << '\n';
	EXPECT_EQ(8, test.error_rate_for_position_per_tile_reference_.at(1).at(3)[0][99][0]) << "SRR490124-4pairs error_rate_for_position_per_tile_reference_[1] wrong for " << context << '\n';

	// echo "count seg ref dom tile rate qual"; cat <(samtools view -q 10 ecoli-SRR490124-4pairs.sam | awk '(int($2%32/16)){print "@" substr($18,6,length($18)-5), $2; print $10; print "+";print $11}' | seqtk seq -r | awk '(1==NR%4){md=substr($1,2,length($0)-1); flag=$2}(2==NR%4){seq=$0}(0==NR%4){printf("%i ", flag);num=0;mult=1;for(i=length(md);0<i;i-=1){b=substr(md,i,1);if(b ~ /^[0-9]/){num+=b*mult;mult*=10}else{base=N;if("A"==b){base="T"}; if("C"==b){base="G"}; if("G"==b){base="C"}; if("T"==b){base="A"}; printf("%i%s", num, base); num=0; mult=1}}; print num, seq, $0}') <(samtools view -q 10 ecoli-SRR490124-4pairs.sam | awk '(!int($2%32/16)){print $2, substr($18,6,length($18)-5), $10, $11}' ) | awk 'BEGIN{for(n=0;n<256;n++)ord[sprintf("%c",n)]=n}{pos=0; num=0; for(i=1;i<=length($2);i+=1){b=substr($2,i,1);if(b ~ /^[0-9]/){num=num*10+b}else{while(0 <= num){pos += 1; num -= 1; if(0>num){ref=b}else{ref=substr($3,pos,1)}; if(0>num && NR%3 != 1){dom=substr($3,pos,1);rate=100}else{dom="N";rate=0}; print int($1%256/128), ref, dom, 0, rate, ord[substr($4,pos,1)]-33}; num=0}}; for(pos+=1;pos<=length($3);pos+=1){print int($1%256/128), substr($3,pos,1), "N", 0, 0, ord[substr($4,pos,1)]-33}}' | sort -n | uniq -c
	EXPECT_EQ(8, test.base_quality_for_error_rate_per_tile_per_error_reference_.at(0).at(1).at(2)[0][100][2]) << "SRR490124-4pairs base_quality_for_error_rate_per_tile_per_error_reference_[0] not correct for " << context << '\n';
	EXPECT_EQ(118, test.base_quality_for_error_rate_per_tile_per_error_reference_.at(0).at(2).at(4)[0][0][2]) << "SRR490124-4pairs base_quality_for_error_rate_per_tile_per_error_reference_[0] not correct for " << context << '\n';
	EXPECT_EQ(8, test.base_quality_for_error_rate_per_tile_per_error_reference_.at(1).at(0).at(4)[0][0][32]) << "SRR490124-4pairs base_quality_for_error_rate_per_tile_per_error_reference_[1] not correct for " << context << '\n';
	EXPECT_EQ(192, test.base_quality_for_error_rate_per_tile_per_error_reference_.at(1).at(3).at(4)[0][0][2]) << "SRR490124-4pairs base_quality_for_error_rate_per_tile_per_error_reference_[1] not correct for " << context << '\n';
	...
	EXPECT_EQ(9, test.base_quality_for_preceding_quality_per_tile_reference_.at(0).at(1)[0][37][37]) << "SRR490124-4pairs base_quality_for_preceding_quality_per_tile_reference_[0] wrong for " << context << '\n';
	EXPECT_EQ(9, test.base_quality_for_preceding_quality_per_tile_reference_.at(0).at(1)[0][37][38]) << "SRR490124-4pairs base_quality_for_preceding_quality_per_tile_reference_[0] wrong for " << context << '\n';
	...
	EXPECT_EQ(9, test.base_quality_for_preceding_quality_per_tile_reference_.at(1).at(2)[0][33][36]) << "SRR490124-4pairs base_quality_for_preceding_quality_per_tile_reference_[1] wrong for " << context << '\n';

	// echo "count seg ref tile rate prev"; cat <(samtools view -q 10 ecoli-SRR490124-4pairs.sam | awk '(int($2%32/16)){print "@" substr($18,6,length($18)-5), $2; print $10; print "+";print $11}' | seqtk seq -r | awk '(1==NR%4){md=substr($1,2,length($0)-1); flag=$2}(2==NR%4){seq=$0}(0==NR%4){printf("%i ", flag);num=0;mult=1;for(i=length(md);0<i;i-=1){b=substr(md,i,1);if(b ~ /^[0-9]/){num+=b*mult;mult*=10}else{base=N;if("A"==b){base="T"}; if("C"==b){base="G"}; if("G"==b){base="C"}; if("T"==b){base="A"}; printf("%i%s", num, base); num=0; mult=1}}; print num, seq, $0}') <(samtools view -q 10 ecoli-SRR490124-4pairs.sam | awk '(!int($2%32/16)){print $2, substr($18,6,length($18)-5), $10, $11}' ) | awk 'BEGIN{for(n=0;n<256;n++)ord[sprintf("%c",n)]=n}{pos=0; num=0; for(i=1;i<=length($2);i+=1){b=substr($2,i,1);if(b ~ /^[0-9]/){num=num*10+b}else{while(0 <= num){pos += 1; num -= 1; if(0>num){ref=b}else{ref=substr($3,pos,1)}; if(0>num && NR%3 != 1){rate=100}else{rate=0}; if(1==pos){prev=1}else{prev=ord[substr($4,pos-1,1)]-33};print int($1%256/128), ref, 0, rate, prev}; num=0}}; for(pos+=1;pos<=length($3);pos+=1){print int($1%256/128), substr($3,pos,1), 0, 0, ord[substr($4,pos-1,1)]-33}}' | sort -n | uniq -c
	...
	EXPECT_EQ(51, test.preceding_quality_for_error_rate_per_tile_reference_.at(0).at(1)[0][0][2]) << "SRR490124-4pairs preceding_quality_for_error_rate_per_tile_reference_[0] wrong for " << context << '\n';
	EXPECT_EQ(28, test.preceding_quality_for_error_rate_per_tile_reference_.at(0).at(2)[0][0][38]) << "SRR490124-4pairs preceding_quality_for_error_rate_per_tile_reference_[0] wrong for " << context << '\n';
	...
	EXPECT_EQ(40, test.preceding_quality_for_error_rate_per_tile_reference_.at(1).at(3)[0][100][2]) << "SRR490124-4pairs preceding_quality_for_error_rate_per_tile_reference_[1] wrong for " << context << '\n';
	...
	EXPECT_EQ(8, test.preceding_quality_for_position_per_tile_reference_.at(0).at(0)[0][6][38]) << "SRR490124-4pairs preceding_quality_for_position_per_tile_reference_[0] wrong for " << context << '\n';
	...
	EXPECT_EQ(8, test.preceding_quality_for_position_per_tile_reference_.at(0).at(2)[0][4][38]) << "SRR490124-4pairs preceding_quality_for_position_per_tile_reference_[0] wrong for " << context << '\n';
	EXPECT_EQ(8, test.preceding_quality_for_position_per_tile_reference_.at(1).at(2)[0][0][1]) << "SRR490124-4pairs preceding_quality_for_position_per_tile_reference_[1] wrong for " << context << '\n';
	...
	EXPECT_EQ(8, test.preceding_quality_for_position_per_tile_reference_.at(1).at(3)[0][99][2]) << "SRR490124-4pairs preceding_quality_for_position_per_tile_reference_[1] wrong for " << context << '\n';
	...
	EXPECT_EQ(48, test.base_quality_for_sequence_quality_per_tile_reference_.at(0).at(3)[0][19][2]) << "SRR490124-4pairs base_quality_for_sequence_quality_per_tile_reference_[0] wrong for " << context << '\n';
	...
	EXPECT_EQ(216, test.base_quality_for_sequence_quality_per_tile_reference_.at(1).at(3)[0][13][2]) << "SRR490124-4pairs base_quality_for_sequence_quality_per_tile_reference_[1] wrong for " << context << '\n';
	...
	EXPECT_EQ(16, test.preceding_quality_for_sequence_quality_per_tile_reference_.at(1).at(1)[0][13][39]) << "SRR490124-4pairs preceding_quality_for_sequence_quality_per_tile_reference_[1] wrong for " << context << '\n';

	// echo "count seg ref tile rate sq"; cat <(samtools view -q 10 ecoli-SRR490124-4pairs.sam | awk '(int($2%32/16)){print "@" substr($18,6,length($18)-5), $2; print $10; print "+";print $11}' | seqtk seq -r | awk '(1==NR%4){md=substr($1,2,length($0)-1); flag=$2}(2==NR%4){seq=$0}(0==NR%4){printf("%i ", flag);num=0;mult=1;for(i=length(md);0<i;i-=1){b=substr(md,i,1);if(b ~ /^[0-9]/){num+=b*mult;mult*=10}else{base=N;if("A"==b){base="T"}; if("C"==b){base="G"}; if("G"==b){base="C"}; if("T"==b){base="A"}; printf("%i%s", num, base); num=0; mult=1}}; print num, seq, $0}') <(samtools view -q 10 ecoli-SRR490124-4pairs.sam | awk '(!int($2%32/16)){print $2, substr($18,6,length($18)-5), $10, $11}' ) | awk 'BEGIN{for(n=0;n<256;n++)ord[sprintf("%c",n)]=n}{sq=0;for(pos=1;pos<=length($4);pos+=1){sq+=ord[substr($4,pos,1)]-33}; sq=int(sq/length($4)+0.5); pos=0; num=0; for(i=1;i<=length($2);i+=1){b=substr($2,i,1);if(b ~ /^[0-9]/){num=num*10+b}else{while(0 <= num){pos += 1; num -= 1; if(0>num){ref=b}else{ref=substr($3,pos,1)}; if(0>num && NR%3 != 1){rate=100}else{rate=0}; print int($1%256/128), ref, 0, rate, sq}; num=0}}; for(pos+=1;pos<=length($3);pos+=1){print int($1%256/128), substr($3,pos,1), 0, 0, sq}}' | sort -n | uniq -c
	...
	EXPECT_EQ(168, test.sequence_quality_for_error_rate_per_tile_reference_.at(0).at(1)[0][0][19]) << "SRR490124-4pairs sequence_quality_for_error_rate_per_tile_reference_[0] wrong for " << context << '\n';
	EXPECT_EQ(27, test.sequence_quality_for_error_rate_per_tile_reference_.at(0).at(2)[0][0][20]) << "SRR490124-4pairs sequence_quality_for_error_rate_per_tile_reference_[0] wrong for " << context << '\n';
	EXPECT_EQ(1, test.sequence_quality_for_error_rate_per_tile_reference_.at(1).at(1)[0][100].size()) << "SRR490124-4pairs sequence_quality_for_error_rate_per_tile_reference_[1] wrong for " << context << '\n';
	EXPECT_EQ(40, test.sequence_quality_for_error_rate_per_tile_reference_.at(1).at(3)[0][100][13]) << "SRR490124-4pairs sequence_quality_for_error_rate_per_tile_reference_[1] wrong for " << context << '\n';
	...
	EXPECT_EQ(8, test.sequence_quality_for_position_per_tile_reference_.at(0).at(1)[0][0][19]) << "SRR490124-4pairs sequence_quality_for_position_per_tile_reference_[0] wrong for " << context << '\n';
	...
	EXPECT_EQ(8, test.sequence_quality_for_position_per_tile_reference_.at(0).at(0)[0][99][19]) << "SRR490124-4pairs sequence_quality_for_position_per_tile_reference_[0] wrong for " << context << '\n';
	...
	EXPECT_EQ(8, test.sequence_quality_for_position_per_tile_reference_.at(1).at(3)[0][99][13]) << "SRR490124-4pairs sequence_quality_for_position_per_tile_reference_[1] wrong for " << context << '\n';
	...
	EXPECT_EQ(8, test.sequence_quality_mean_for_gc_per_tile_reference_.at(0)[0][57][19] ) << "SRR490124-4pairs sequence_quality_mean_for_gc_per_tile_reference_[0][0][57] wrong for " << context << '\n';
	...
	EXPECT_EQ(8, test.sequence_quality_mean_for_gc_per_tile_reference_.at(1)[0][46][13] ) << "SRR490124-4pairs sequence_quality_mean_for_gc_per_tile_reference_[1][0][37] wrong for " << context << '\n';
	...
	EXPECT_EQ(7, test.sequence_quality_mean_for_fragment_length_per_tile_reference_.at(0)[0].size() ) << "SRR490124-4pairs sequence_quality_mean_for_fragment_length_per_tile_reference_[0][0].size() wrong for " << context << '\n';
	EXPECT_EQ(1, test.sequence_quality_mean_for_fragment_length_per_tile_reference_.at(0)[0][126/10][20] ) << "SRR490124-4pairs sequence_quality_mean_for_fragment_length_per_tile_reference_[0][0][126] wrong for " << context << '\n';
	EXPECT_EQ(8, test.sequence_quality_mean_for_fragment_length_per_tile_reference_.at(0)[0][184/10][19] ) << "SRR490124-4pairs sequence_quality_mean_for_fragment_length_per_tile_reference_[0][0][184] wrong for " << context << '\n';
	EXPECT_EQ(7, test.sequence_quality_mean_for_fragment_length_per_tile_reference_.at(1)[0].size() ) << "SRR490124-4pairs sequence_quality_mean_for_fragment_length_per_tile_reference_[1][0].size() wrong for " << context << '\n';
	EXPECT_EQ(8, test.sequence_quality_mean_for_fragment_length_per_tile_reference_.at(1)[0][184/10][13] ) << "SRR490124-4pairs sequence_quality_mean_for_fragment_length_per_tile_reference_[1][0][184] wrong for " << context << '\n';
	EXPECT_EQ(1, test.sequence_quality_mean_for_fragment_length_per_tile_reference_.at(1)[0][126/10][12] ) << "SRR490124-4pairs sequence_quality_mean_for_fragment_length_per_tile_reference_[1][0][126] wrong for " << context << '\n';
	...
	EXPECT_EQ(1, test.gc_for_fragment_length_per_tile_reference_.at(0)[0][126/10][54] ) << "SRR490124-4pairs gc_for_fragment_length_per_tile_reference_[0][0][126] wrong for " << context << '\n';
	EXPECT_EQ(8, test.gc_for_fragment_length_per_tile_reference_.at(0)[0][184/10][57] ) << "SRR490124-4pairs gc_for_fragment_length_per_tile_reference_[0][0][184] wrong for " << context << '\n';
	EXPECT_EQ(7, test.gc_for_fragment_length_per_tile_reference_.at(1)[0].size() ) << "SRR490124-4pairs gc_for_fragment_length_per_tile_reference_[1][0].size() wrong for " << context << '\n';
	EXPECT_EQ(8, test.gc_for_fragment_length_per_tile_reference_.at(1)[0][184/10][46] ) << "SRR490124-4pairs gc_for_fragment_length_per_tile_reference_[1][0][184] wrong for " << context << '\n';
	EXPECT_EQ(1, test.gc_for_fragment_length_per_tile_reference_.at(1)[0][126/10][52] ) << "SRR490124-4pairs gc_for_fragment_length_per_tile_reference_[1][0][126] wrong for " << context << '\n';
	...
	EXPECT_EQ(56, test.preceding_quality_for_sequence_per_tile_.at(0).at(1)[0][19][38]) << "SRR490124-4pairs preceding_quality_for_sequence_per_tile_[0] wrong for " << context << '\n';
	EXPECT_EQ(128, test.preceding_quality_for_sequence_per_tile_.at(0).at(2)[0][19][2]) << "SRR490124-4pairs preceding_quality_for_sequence_per_tile_[0] wrong for " << context << '\n';
	...
	EXPECT_EQ(8, test.preceding_quality_for_sequence_per_tile_.at(1).at(1)[0][21][39]) << "SRR490124-4pairs preceding_quality_for_sequence_per_tile_[1] wrong for " << context << '\n';
	EXPECT_EQ(40, test.preceding_quality_for_sequence_per_tile_.at(1).at(3)[0][21][2]) << "SRR490124-4pairs preceding_quality_for_sequence_per_tile_[1] wrong for " << context << '\n';
	...
	EXPECT_EQ(8, test.preceding_quality_for_position_per_tile_.at(0).at(0)[0][4][39]) << "SRR490124-4pairs preceding_quality_for_position_per_tile_[0] wrong for " << context << '\n';
	...
	EXPECT_EQ(8, test.preceding_quality_for_position_per_tile_.at(0).at(2)[0][4][38]) << "SRR490124-4pairs preceding_quality_for_position_per_tile_[0] wrong for " << context << '\n';
	EXPECT_EQ(8, test.preceding_quality_for_position_per_tile_.at(1).at(2)[0][0][1]) << "SRR490124-4pairs preceding_quality_for_position_per_tile_[1] wrong for " << context << '\n';
	...
	EXPECT_EQ(16, test.preceding_quality_for_position_per_tile_.at(1).at(3)[0][99][2]) << "SRR490124-4pairs preceding_quality_for_position_per_tile_[1] wrong for " << context << '\n';
	...
	EXPECT_EQ(8, test.sequence_quality_for_position_per_tile_.at(0).at(0)[0][0][20]) << "SRR490124-4pairs sequence_quality_for_position_per_tile_[0] wrong for " << context << '\n';
	EXPECT_EQ(8, test.sequence_quality_for_position_per_tile_.at(0).at(0)[0][99][19]) << "SRR490124-4pairs sequence_quality_for_position_per_tile_[0] wrong for " << context << '\n';
	EXPECT_EQ(8, test.sequence_quality_for_position_per_tile_.at(0).at(1)[0][0][19]) << "SRR490124-4pairs sequence_quality_for_position_per_tile_[0] wrong for " << context << '\n';
	EXPECT_EQ(9, test.sequence_quality_for_position_per_tile_.at(0).at(3)[0][29][20]) << "SRR490124-4pairs sequence_quality_for_position_per_tile_[0] wrong for " << context << '\n';
	EXPECT_EQ(8, test.sequence_quality_for_position_per_tile_.at(1).at(3)[0][99][13]) << "SRR490124-4pairs sequence_quality_for_position_per_tile_[1] wrong for " << context << '\n';
	EXPECT_EQ(8, test.sequence_quality_for_position_per_tile_.at(1).at(3)[0][99][21]) << "SRR490124-4pairs sequence_quality_for_position_per_tile_[1] wrong for " << context << '\n';
	...
	EXPECT_EQ(8, test.sequence_quality_for_base_per_tile_.at(0).at(0)[0][24][20] ) << "SRR490124-4pairs sequence_quality_for_base_per_tile_[0][0][0][24] wrong for " << context << '\n';
	...
	EXPECT_EQ(8, test.sequence_quality_for_base_per_tile_.at(0).at(0)[0][25][19] ) << "SRR490124-4pairs sequence_quality_for_base_per_tile_[0][0][0][25] wrong for " << context << '\n';
	...
	EXPECT_EQ(8, test.sequence_quality_for_base_per_tile_.at(0).at(1)[0][25][19] ) << "SRR490124-4pairs sequence_quality_for_base_per_tile_[0][1][0][25] wrong for " << context << '\n';
	...
	EXPECT_EQ(8, test.sequence_quality_for_base_per_tile_.at(0).at(1)[0][32][20] ) << "SRR490124-4pairs sequence_quality_for_base_per_tile_[0][1][0][32] wrong for " << context << '\n';
	EXPECT_EQ(8, test.sequence_quality_for_base_per_tile_.at(0).at(2)[0][20][20] ) << "SRR490124-4pairs sequence_quality_for_base_per_tile_[0][1][0][20] wrong for " << context << '\n';
	EXPECT_EQ(8, test.sequence_quality_for_base_per_tile_.at(0).at(2)[0][34][19] ) << "SRR490124-4pairs sequence_quality_for_base_per_tile_[0][1][0][34] wrong for " << context << '\n';
	EXPECT_EQ(8, test.sequence_quality_for_base_per_tile_.at(0).at(3)[0][16][19] ) << "SRR490124-4pairs sequence_quality_for_base_per_tile_[0][1][0][16] wrong for " << context << '\n';
	EXPECT_EQ(8, test.sequence_quality_for_base_per_tile_.at(0).at(3)[0][24][20] ) << "SRR490124-4pairs sequence_quality_for_base_per_tile_[0][1][0][24] wrong for " << context << '\n';
	...
	EXPECT_EQ(8, test.sequence_quality_for_base_per_tile_.at(1).at(3)[0][25][21] ) << "SRR490124-4pairs sequence_quality_for_base_per_tile_[1][3][0][25] wrong for " << context << '\n';
	...
	EXPECT_EQ(8, test.sequence_quality_for_base_per_tile_.at(1).at(3)[0][32][13] ) << "SRR490124-4pairs sequence_quality_for_base_per_tile_[1][3][0][32] wrong for " << context << '\n';
	...
	EXPECT_EQ(8, test.sequence_quality_mean_for_gc_per_tile_.at(0)[0][52][20] ) << "SRR490124-4pairs sequence_quality_mean_for_gc_per_tile_[0][0][52] wrong for " << context << '\n';
	EXPECT_EQ(8, test.sequence_quality_mean_for_gc_per_tile_.at(0)[0][59][19] ) << "SRR490124-4pairs sequence_quality_mean_for_gc_per_tile_[0][0][59] wrong for " << context << '\n';
	...
	EXPECT_EQ(8, test.sequence_quality_mean_for_gc_per_tile_.at(1)[0][37][21] ) << "SRR490124-4pairs sequence_quality_mean_for_gc_per_tile_[1][0][37] wrong for " << context << '\n';
	EXPECT_EQ(8, test.sequence_quality_mean_for_gc_per_tile_.at(1)[0][47][13] ) << "SRR490124-4pairs sequence_quality_mean_for_gc_per_tile_[1][0][47] wrong for " << context << '\n';
	...
	EXPECT_EQ(8, test.sequence_quality_probability_mean_.at(0)[5] ) << "SRR490124-4pairs sequence_quality_probability_mean_ wrong for " << context << '\n';
	EXPECT_EQ(9, test.sequence_quality_probability_mean_.at(0)[6] ) << "SRR490124-4pairs sequence_quality_probability_mean_ wrong for " << context << '\n';
	...
	EXPECT_EQ(9, test.sequence_quality_probability_mean_.at(1)[4] ) << "SRR490124-4pairs sequence_quality_probability_mean_ wrong for " << context << '\n';
	EXPECT_EQ(8, test.sequence_quality_probability_mean_.at(1)[6] ) << "SRR490124-4pairs sequence_quality_probability_mean_ wrong for " << context << '\n';
	...
	for( int templ_seg=2; templ_seg--; ){
		...
		EXPECT_EQ(18, test.sequence_quality_minimum_.at(templ_seg)[2]) << "SRR490124-4pairs sequence_quality_minimum_[" << templ_seg << "] wrong for " << context << '\n';
		...
		EXPECT_EQ(18, test.sequence_quality_first_quartile_.at(templ_seg)[2]) << "SRR490124-4pairs sequence_quality_first_quartile_[" << templ_seg << "] wrong for " << context << '\n';
	}
	EXPECT_EQ(26, test.sequence_quality_median_.at(0).size()) << "SRR490124-4pairs sequence_quality_median_[0] wrong for " << context << '\n';
	EXPECT_EQ(16, test.sequence_quality_median_.at(0)[26]) << "SRR490124-4pairs sequence_quality_median_[0] wrong for " << context << '\n';
	EXPECT_EQ(26, test.sequence_quality_median_.at(1).size()) << "SRR490124-4pairs sequence_quality_median_[1] wrong for " << context << '\n';
	EXPECT_EQ(10, test.sequence_quality_median_.at(1)[2]) << "SRR490124-4pairs sequence_quality_median_[1] wrong for " << context << '\n';
	EXPECT_EQ(8, test.sequence_quality_median_.at(1)[27]) << "SRR490124-4pairs sequence_quality_median_[1] wrong for " << context << '\n';
	...
	EXPECT_EQ(16, test.sequence_quality_third_quartile_.at(0)[36]) << "SRR490124-4pairs sequence_quality_third_quartile_[0] wrong for " << context << '\n';
	...
	EXPECT_EQ(8, test.sequence_quality_third_quartile_.at(1)[29]) << "SRR490124-4pairs sequence_quality_third_quartile_[1] wrong for " << context << '\n';
	EXPECT_EQ(8, test.sequence_quality_third_quartile_.at(1)[36]) << "SRR490124-4pairs sequence_quality_third_quartile_[1] wrong for " << context << '\n';
	EXPECT_EQ(3, test.sequence_quality_maximum_.at(0).size()) << "SRR490124-4pairs sequence_quality_maximum_[0] wrong for " << context << '\n';
	EXPECT_EQ(9, test.sequence_quality_maximum_.at(0)[38]) << "SRR490124-4pairs sequence_quality_maximum_[0] wrong for " << context << '\n';
	EXPECT_EQ(8, test.sequence_quality_maximum_.at(0)[39]) << "SRR490124-4pairs sequence_quality_maximum_[0] wrong for " << context << '\n';
	EXPECT_EQ(2, test.sequence_quality_maximum_.at(1).size()) << "SRR490124-4pairs sequence_quality_maximum_[1] wrong for " << context << '\n';
	EXPECT_EQ(17, test.sequence_quality_maximum_.at(1)[39]) << "SRR490124-4pairs sequence_quality_maximum_[1] wrong for " << context << '\n';
	...
	TestVectEquality({0,{9,1,8}}, test.sequence_quality_content_.at(0)[20], context, "SRR490124-4pairs sequence_quality_content_[0][20]", " wrong for ");
	...
	TestVectEquality({0,{10,8}}, test.sequence_quality_content_.at(1)[4], context, "SRR490124-4pairs sequence_quality_content_[1][4]", " wrong for ");
	TestVectEquality({0,{1,1,16}}, test.sequence_quality_content_.at(1)[20], context, "SRR490124-4pairs sequence_quality_content_[1][20]", " wrong for ");
	TestVectEquality({0,{1,0,0,0,0,8,0,8,1}}, test.sequence_quality_content_.at(1)[39], context, "SRR490124-4pairs sequence_quality_content_[1][39]", " wrong for ");
	...
	TestVectEquality({1,{8}}, test.homoquality_distribution_[4], context, "SRR490124-4pairs homoquality_distribution_[4]", " not correct for ");
	TestVectEquality({1,{50}}, test.homoquality_distribution_[20], context, "SRR490124-4pairs homoquality_distribution_[20]", " not correct for ");
	TestVectEquality({1,{96,40,9,0,1}}, test.homoquality_distribution_[39], context, "SRR490124-4pairs homoquality_distribution_[39]", " not correct for ");
	...
	TestSeqQualityStats(test.nucleotide_quality_.at(0).at(0), 38, 19, 2, 24, 39, context, "of SRR490124-4pairs nucleotide_quality_[0][0] wrong for ");
	TestSeqQualityStats(test.nucleotide_quality_.at(0).at(1), 38, 21, 2, 29, 39, context, "of SRR490124-4pairs nucleotide_quality_[0][1] wrong for ");
	TestSeqQualityStats(test.nucleotide_quality_.at(0).at(2), 38, 18, 2, 17, 39, context, "of SRR490124-4pairs nucleotide_quality_[0][2] wrong for ");
	TestSeqQualityStats(test.nucleotide_quality_.at(0).at(3), 38, 18, 2, 20, 39, context, "of SRR490124-4pairs nucleotide_quality_[0][3] wrong for ");
	EXPECT_EQ(0, test.nucleotide_quality_.at(0).at(4).size()) << "size of SRR490124-4pairs nucleotide_quality_[0][4] wrong for " << context << '\n';
	TestSeqQualityStats(test.nucleotide_quality_.at(1).at(0), 38, 21, 2, 31, 39, context, "of SRR490124-4pairs nucleotide_quality_[1][0] wrong for ");
	TestSeqQualityStats(test.nucleotide_quality_.at(1).at(1), 38, 13, 2, 2, 39, context, "of SRR490124-4pairs nucleotide_quality_[1][1] wrong for ");
	TestSeqQualityStats(test.nucleotide_quality_.at(1).at(2), 38, 15, 2, 2, 39, context, "of SRR490124-4pairs nucleotide_quality_[1][2] wrong for ");
	TestSeqQualityStats(test.nucleotide_quality_.at(1).at(3), 38, 14, 2, 2, 39, context, "of SRR490124-4pairs nucleotide_quality_[1][3] wrong for ");
	...
	EXPECT_EQ(360, test.base_quality_for_sequence_.at(0)[19][2]) << "SRR490124-4pairs base_quality_for_sequence_[0] wrong for " << context << '\n';
	...
	EXPECT_EQ(272, test.base_quality_for_sequence_.at(1)[21][2]) << "SRR490124-4pairs base_quality_for_sequence_[1] wrong for " << context << '\n';
	...
	EXPECT_EQ(1800, SumVect(test.base_quality_for_preceding_quality_.at(0))) << "SRR490124-4pairs base_quality_for_preceding_quality_[0] wrong for " << context << '\n';
	...
	EXPECT_EQ(8, test.base_quality_for_preceding_quality_.at(0)[37][27]) << "SRR490124-4pairs base_quality_for_preceding_quality_[0] wrong for " << context << '\n';
	EXPECT_EQ(9, test.base_quality_for_preceding_quality_.at(0)[37][34]) << "SRR490124-4pairs base_quality_for_preceding_quality_[0] wrong for " << context << '\n';
	EXPECT_EQ(18, test.base_quality_for_preceding_quality_.at(0)[37][35]) << "SRR490124-4pairs base_quality_for_preceding_quality_[0] wrong for " << context << '\n';
	EXPECT_EQ(12, test.base_quality_for_preceding_quality_.at(0)[37][36]) << "SRR490124-4pairs base_quality_for_preceding_quality_[0] wrong for " << context << '\n';
	EXPECT_EQ(16, test.base_quality_for_preceding_quality_.at(0)[37][37]) << "SRR490124-4pairs base_quality_for_preceding_quality_[0] wrong for " << context << '\n';
	EXPECT_EQ(18, test.base_quality_for_preceding_quality_.at(0)[37][38]) << "SRR490124-4pairs base_quality_for_preceding_quality_[0] wrong for " << context << '\n';
	EXPECT_EQ(24, test.base_quality_for_preceding_quality_.at(0)[37][39]) << "SRR490124-4pairs base_quality_for_preceding_quality_[0] wrong for " << context << '\n';
	EXPECT_EQ(1800, SumVect(test.base_quality_for_preceding_quality_.at(1))) << "SRR490124-4pairs base_quality_for_preceding_quality_[1] wrong for " << context << '\n';
	...
	EXPECT_EQ(8, test.base_quality_for_preceding_quality_.at(1)[33][2]) << "SRR490124-4pairs base_quality_for_preceding_quality_[1] wrong for " << context << '\n';
	EXPECT_EQ(8, test.base_quality_for_preceding_quality_.at(1)[33][24]) << "SRR490124-4pairs base_quality_for_preceding_quality_[1] wrong for " << context << '\n';
	EXPECT_EQ(26, test.base_quality_for_preceding_quality_.at(1)[33][36]) << "SRR490124-4pairs base_quality_for_preceding_quality_[1] wrong for " << context << '\n';
	EXPECT_EQ(8, test.base_quality_for_preceding_quality_.at(1)[33][37]) << "SRR490124-4pairs base_quality_for_preceding_quality_[1] wrong for " << context << '\n';
	EXPECT_EQ(16, test.base_quality_for_preceding_quality_.at(1)[33][39]) << "SRR490124-4pairs base_quality_for_preceding_quality_[1] wrong for " << context << '\n';
	...
	EXPECT_EQ(8, test.sequence_quality_mean_.at(0)[19]) << "SRR490124-4pairs sequence_quality_mean_[0] wrong for " << context << '\n';
	EXPECT_EQ(9, test.sequence_quality_mean_.at(0)[20]) << "SRR490124-4pairs sequence_quality_mean_[0] wrong for " << context << '\n';
	EXPECT_EQ(18, test.sequence_quality_mean_.at(1).size()) << "SRR490124-4pairs sequence_quality_mean_[1] wrong for " << context << '\n';
	EXPECT_EQ(8, test.sequence_quality_mean_.at(1)[13]) << "SRR490124-4pairs sequence_quality_mean_[1] wrong for " << context << '\n';
	EXPECT_EQ(8, test.sequence_quality_mean_.at(1)[21]) << "SRR490124-4pairs sequence_quality_mean_[1] wrong for " << context << '\n';
	for( int templ_seg=2; templ_seg--; ){
...
void QualityStatsTest::TestVariants(const QualityStats &test){
	...
	EXPECT_EQ(8, test.base_quality_stats_per_tile_per_error_reference_.at(0).at(0).at(1)[0][70][2]);
	EXPECT_EQ(8, test.base_quality_stats_per_tile_per_error_reference_.at(0).at(2).at(1)[0][86][2]);
	EXPECT_EQ(98, test.base_quality_stats_per_tile_per_error_reference_.at(1).at(3).at(4)[0].size());
	EXPECT_EQ(1, test.base_quality_stats_per_tile_per_error_reference_.at(1).at(3).at(4)[0][10].size());
	EXPECT_EQ(8, test.base_quality_stats_per_tile_per_error_reference_.at(1).at(3).at(4)[0][10][39]);

	EXPECT_EQ(8, test.error_rate_for_position_per_tile_per_error_reference_.at(0).at(1).at(2)[0][87][100]);
	EXPECT_EQ(8, test.error_rate_for_position_per_tile_per_error_reference_.at(0).at(2).at(4)[0][97][0]);
	EXPECT_EQ(8, test.error_rate_for_position_per_tile_per_error_reference_.at(1).at(0).at(4)[0][1][0]);
	EXPECT_EQ(0, test.error_rate_for_position_per_tile_per_error_reference_.at(1).at(3).at(4)[0][99][0]);
	EXPECT_EQ(8, test.error_rate_for_position_per_tile_reference_.at(0).at(1)[0][0][0]);
	EXPECT_EQ(0, test.error_rate_for_position_per_tile_reference_.at(0).at(2)[0][0][0]);
	EXPECT_EQ(8, test.error_rate_for_position_per_tile_reference_.at(0).at(0)[0][99][0]);
	EXPECT_EQ(1, test.error_rate_for_position_per_tile_reference_.at(0).at(3)[0][93][0]);
	EXPECT_EQ(0, test.error_rate_for_position_per_tile_reference_.at(1).at(0)[0][99][0]);
	EXPECT_EQ(0, test.error_rate_for_position_per_tile_reference_.at(1).at(3)[0][99][0]);

	EXPECT_EQ(8, test.base_quality_for_error_rate_per_tile_per_error_reference_.at(0).at(1).at(2)[0][100][2]);
	EXPECT_EQ(118, test.base_quality_for_error_rate_per_tile_per_error_reference_.at(0).at(2).at(4)[0][0][2]);
	EXPECT_EQ(8, test.base_quality_for_error_rate_per_tile_per_error_reference_.at(1).at(0).at(4)[0][0][32]);
	...
	EXPECT_EQ(9, test.base_quality_for_preceding_quality_per_tile_reference_.at(0).at(1)[0][37][37]);
	EXPECT_EQ(9, test.base_quality_for_preceding_quality_per_tile_reference_.at(0).at(1)[0][37][38]);
	...
	EXPECT_EQ(9, test.base_quality_for_preceding_quality_per_tile_reference_.at(1).at(2)[0][33][36]);

	EXPECT_EQ(38, test.preceding_quality_for_error_rate_per_tile_reference_.at(0).at(1)[0][0].size());
	EXPECT_EQ(51, test.preceding_quality_for_error_rate_per_tile_reference_.at(0).at(1)[0][0][2]);
	EXPECT_EQ(28, test.preceding_quality_for_error_rate_per_tile_reference_.at(0).at(2)[0][0][38]);
	EXPECT_EQ(1, test.preceding_quality_for_error_rate_per_tile_reference_.at(1).at(1)[0][100].size());
	EXPECT_EQ(40, test.preceding_quality_for_error_rate_per_tile_reference_.at(1).at(3)[0][100][2]);

	EXPECT_EQ(8, test.preceding_quality_for_position_per_tile_reference_.at(0).at(0)[0][6][38]);
	EXPECT_EQ(93, test.preceding_quality_for_position_per_tile_reference_.at(0).at(1)[0].size());
	EXPECT_EQ(8, test.preceding_quality_for_position_per_tile_reference_.at(0).at(2)[0][4][38]);
	EXPECT_EQ(8, test.preceding_quality_for_position_per_tile_reference_.at(1).at(2)[0][0][1]);
	EXPECT_EQ(98, test.preceding_quality_for_position_per_tile_reference_.at(1).at(3)[0].size());
	EXPECT_EQ(0, test.preceding_quality_for_position_per_tile_reference_.at(1).at(3)[0][99][2]);
	...
	EXPECT_EQ(48, test.base_quality_for_sequence_quality_per_tile_reference_.at(0).at(3)[0][19][2]);
	...
	EXPECT_EQ(16, test.preceding_quality_for_sequence_quality_per_tile_reference_.at(1).at(1)[0][13][39]);

	EXPECT_EQ(2, test.sequence_quality_for_error_rate_per_tile_reference_.at(0).at(1)[0][0].size());
	EXPECT_EQ(168, test.sequence_quality_for_error_rate_per_tile_reference_.at(0).at(1)[0][0][19]);
	EXPECT_EQ(26, test.sequence_quality_for_error_rate_per_tile_reference_.at(0).at(2)[0][0][20]);
	EXPECT_EQ(1, test.sequence_quality_for_error_rate_per_tile_reference_.at(1).at(1)[0][100].size());
	EXPECT_EQ(40, test.sequence_quality_for_error_rate_per_tile_reference_.at(1).at(3)[0][100][13]);

	EXPECT_EQ(8, test.sequence_quality_for_position_per_tile_reference_.at(0).at(1)[0][0][19]);
	EXPECT_EQ(0, test.sequence_quality_for_position_per_tile_reference_.at(0).at(2)[0][0][20]);
	EXPECT_EQ(8, test.sequence_quality_for_position_per_tile_reference_.at(0).at(0)[0][99][19]);
	...
	EXPECT_EQ(1, test.sequence_quality_mean_for_fragment_length_per_tile_reference_.at(0)[0][126/10][20] );
	EXPECT_EQ(8, test.sequence_quality_mean_for_fragment_length_per_tile_reference_.at(0)[0][184/10][19] );
	EXPECT_EQ(7, test.sequence_quality_mean_for_fragment_length_per_tile_reference_.at(1)[0].size() );
	EXPECT_EQ(8, test.sequence_quality_mean_for_fragment_length_per_tile_reference_.at(1)[0][184/10][13] );
	EXPECT_EQ(1, test.sequence_quality_mean_for_fragment_length_per_tile_reference_.at(1)[0][126/10][12] );
	...
	TestSizeAtIndex_0_1_98_99(test.base_quality_stats_reference_.at(1), 1, 10, 1, 0, "variant test", "SRR490124-4pairs base_quality_stats_reference_[1] not correctly shrunken for ");
...
void QualityStatsTest::TestCoverage(const QualityStats &test){
	...
	EXPECT_EQ(0, test.base_quality_stats_per_tile_reference_.at(1).at(2)[0][2].size()) << "base_quality_stats_per_tile_reference_[1][2][0][2] not correctly shrunken in coverage test\n";
}
